...
```{r, fig.width=8, fig.height=12}
#* Notice that we don't have to access the "dag" element here
#* The plot.HydeNetwork method knows to do this already.
plot(BlackJack)
```
...
We haven't printed the options here because it's a big ugly list of lists that is a little bit painful to look at.  We'll admit that the process for customizing `Rgraphviz` attributes isn't something we enjoy, and it may seem counterintuitive.  Essentially each Graphviz attribute is a vector.  Each element of the vector is named where each name is a node and each element is the assignment of the attribute for that node.

Let us show you an example.  For the previous plot, the `shape` attribute looks like this:

```
            card1             card2      dealerUpcard             card3 
        "ellipse"         "ellipse"         "ellipse"         "ellipse" 
            card4             card5 dealerFinalPoints         card1.ace 
        "ellipse"         "ellipse"         "ellipse"         "ellipse" 
        card2.ace     initialPoints         card3.ace  pointsAfterCard3 
        "ellipse"         "ellipse"         "ellipse"         "ellipse" 
        card4.ace  pointsAfterCard4         card5.ace  pointsAfterCard5 
        "ellipse"         "ellipse"         "ellipse"         "ellipse" 
playerFinalPoints              hit1              hit2              hit3 
        "ellipse"            "rect"            "rect"            "rect" 
           payoff 
           "rect" 
```

To get the network plot, we need to construct a vector like that for `shape`, `fillcolor`, `fontcolor` and `linecolor`.  Any nodes that aren't named in a vector will adopt `Rgraphviz`'s default (a white circle with black text).

But rather than put you through all of that work, we've put together some functions to make it easier to customize your plots.

For now, let's assume that you only want to change the default scheme.  That is, you want the representation of each node to depend on its type.  Rather than assigning each type to an attribute, it seems more intuitive to assign the attributes to a type.  We can do this using the `HydePlotOptions` function.  To demonstrate, let's change the defaults so that:
...
```{r}
plot(BlackJack,
     hit1 = customNode(height = "2"))
plot(BlackJack,
     hit2 = customNode(height = "2"))
plot(BlackJack,
     hit3 = customNode(height = "2"))
```

The way that the node customization works is that it only applies custom attributes _after_ establishing the plot defaults.
...
```{r}
plot(BlackJack,
     payoff = customNode(fillcolor = "green", shape = "ellipse", 
                         height="2", width="4"),
