...
The first step in the analysis is to set up the network structure. This is done using the `HydeNetwork()` function. Syntax is the same as for the `dag()` command in the `gRbase` library:
...
Evidence on individual nodes can also be entered with `compileJagsModel()`, thus facilitating dynamic inference as data are observed. See the description of the `data` argument in `help(rjags::jags.model)` for details. Note that, here, we reference the observation of a 6-cylinder car (the 3rd level of factor `mtcars2$cyl`) by its numeric code.
