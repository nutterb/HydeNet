...
However, we also allow for utility nodes to be random variables. In that case, we might interpret them as reflecting some degree of uncertainty about their realized values. For example, we might have a dishonest dealer who forgets to pay us when we win sometimes:

```
#update model to add deterministic node 'playerWin'
#update payoff node to represent randomness
```
