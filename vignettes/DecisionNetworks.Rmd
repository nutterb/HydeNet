...
Node distributions for all random variable & deterministic nodes are now specified, other than `dealerFinalPoints`. In the below long and ugly code block, we used logistic regression equations to specify the conditional distribution of `dealerFinalPoints`. These equations are based on the following probabilities (assuming the dealer is drawing from a deck of infinite size; *source: https://www.blackjackinfo.com/dealer-outcome-probabilities. Accessed 2015-04-24.*):
...
**Utility nodes** are restricted to have no child nodes. Typically, utility nodes are deterministic functions of their parent nodes. In the blackjack example, for instance, we would assign the payoff as follows:
...
However, the `HydeNet` package also allows for utility nodes to be random variables. In that case, we might interpret them as reflecting some degree of uncertainty about their realized values. If a child decides to steal a candy bar from the store, the utility might randomly depend on whether or not (s)he gets caught.
...
We will analyze the following four strategies: never choose to "hit", always take 3 cards, always take 4 cards, and always take 5 cards.
