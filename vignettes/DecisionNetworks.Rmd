...
To begin the modeling, we first construct a skeleton `HydeNet` model object, (i.e., a graphical model for which node characteristics have not yet been populated -- see vignette titled, "Working with HydeNetwork Objects"):
...
We briefly defer visualization of the network structure until after we've specified the types (random variable, deterministic, decision, or utility) of all the nodes in the network, since `plot.HydeNetwork()` allows for enhanced visualization of decision networks. See below under "Visualization of Decision Networks".
